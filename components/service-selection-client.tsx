...
export function ServiceSelectionClient({ plateFromPath }: { plateFromPath?: string }) {
  const search = useSearchParams();
  const router = useRouter();
  const plate = plateFromPath ?? search.get("plate") ?? "";
  ...
  useEffect(() => {
    if (plate) api<Vehicle>(`/api/vehicles/${encodeURIComponent(plate)}`).then(setVehicle);
